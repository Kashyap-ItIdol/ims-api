...
namespace IMS_Application.Services
{
    public class SubCategoryService : ISubCategoryService
    {
        ...
        public async Task<Result<List<SubCategoryDto>>> GetAllSubCategoriesAsync()
        {
            try
            {
                ...
                return Result<List<SubCategoryDto>>.Success(subCategoryDtos, "Sub-categories retrieved successfully");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving all sub categories");
                return Result<List<SubCategoryDto>>.Failure(ErrorMessages.UnexpectedError, 500);
            }
        }

        public async Task<Result<List<SubCategoryDto>>> GetSubCategoriesByCategoryIdAsync(int categoryId)
        {
            try
            {
                if (categoryId <= 0)
        ...
        public async Task<Result<SubCategoryDto>> UpdateSubCategoryAsync(int id, UpdateSubCategoryDto request, int updatedBy)
        {
            try
            {
                if (id <= 0)
                ...
                var updatedDto = _mapper.Map<SubCategoryDto>(existingSubCategory);
                return Result<SubCategoryDto>.Success(updatedDto, "Sub-category updated successfully");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error updating sub category with ID: {SubCategoryId}", id);
                return Result<SubCategoryDto>.Failure(ErrorMessages.UnexpectedError, 500);
            }
        }
